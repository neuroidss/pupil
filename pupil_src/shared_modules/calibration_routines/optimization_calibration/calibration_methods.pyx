...
def five_point_algo_calibration( sphere_position, ref_directions_3D, gaze_directions_3D , initial_orientation , initial_translation ,
    fix_translation = False, translation_lower_bound = (15,5,5) ,  translation_upper_bound = (15,5,5) ):


    cdef vector[Point3d] cpp_ref_directions
    cdef vector[Point3d] cpp_gaze_directions
    for p in ref_directions_3D:
        cpp_ref_directions.push_back(Point3d(p[0],p[1],p[2]))

    for p in gaze_directions_3D:
        cpp_gaze_directions.push_back(Point3d(p[0],p[1],p[2]))
    ...
    ## optimized values are written to cpp_orientation and cpp_translation
    cdef bint success  = fivePointAlgoCalibration(cpp_sphere_position, cpp_ref_directions, cpp_gaze_directions,
                                             &cpp_orientation[0], &cpp_translation[0], fix_translation,
                                             cpp_translation_lower_bound, cpp_translation_upper_bound )
